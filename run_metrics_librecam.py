...
from benchmark.pascal_run_competing_saliency_librecam import get_image_tensor
#%%
available_metrics = [  
                    'perturbation',
                    'road',
    ...
    for methodname in methodnames:
        for metricname in metrics:
            ...
            if len(im_save_dirs) == 0:
                # import colorful
                print(colorful.magenta(f'im_save_dirs is empty,methoddir:{methoddir}'))
                import ipdb;ipdb.set_trace()
            ...
            for i,im_save_dir in enumerate(im_save_dirs):
                ...
                try:
                    ...
                    # except EOFError as e:
                        ...
                        if os.environ.get('DBG_PKL_EOF',False)=='1':
                            # import ipdb;ipdb.set_trace()
                            pass
                        else:
                            raise e
                ...
                if dataset in ['imagenet','']:
                    ...
                    # target_id = class_names.index(classname)
                    classname = classname.replace(' ','_')                    
                else:
                    assert False, 'dataset not recognized'
                #--------------------------------------------------------------------
                if skip:
                    save_dir = os.path.join(settings.RESULTS_DIR_librecam,f'{dataset}-{metricname}-{methodname}-{modelname}',imroot)
                    if USE_LZMA:
                ...
                #----------------------------------------------------------
                # avverage metrics
                # from benchmark.analysis_utils import get_avg_metrics
                def update_running_avg(new_sample,running_averages,metricnames):
                    per_metric = {}

                    if len(running_averages) == 0:
                        running_averages['n_total'] = 0
                        
                    for k in metricnames:
                        v = running_averages.get(k,0)
                        t = running_averages['n_total']
                        running_averages[k] = v + (new_sample[k] - v)/(t+1)
                        assert not np.isnan(running_averages[k])
                        assert not np.isinf(running_averages[k])
                    running_averages['n_total'] += 1
